...
export const frameworkMasteryProblems: Record<string, Problem[]> = {
  ...
  flask: makeMasteryTrack('flask', [
    { title: 'Flask Mental Model', focus: 'Flask as a minimal WSGI framework: app object, routes, request/response, extensions, and explicit architecture choices', task: 'draw a Flask request path and identify what Flask gives you versus what you must design yourself.' },
    { title: 'WSGI And Gunicorn', focus: 'WSGI callables, synchronous workers, threads, worker counts, request blocking, and deployment shape', task: 'explain how a Flask app runs under Gunicorn and why one blocked worker matters.' },
    { title: 'Application Factory', focus: 'create_app, delayed setup, testable app instances, environment-specific config, and avoiding import-time side effects', task: 'build a create_app(config) factory that registers routes and extensions.' },
    { title: 'Blueprint Organization', focus: 'splitting routes by domain, registering URL prefixes, avoiding circular imports, and keeping feature boundaries clean', task: 'move users and projects routes into separate blueprints.' },
    { title: 'Configuration Objects', focus: 'dev/test/prod config, environment variables, secrets, defaults, and fail-fast validation', task: 'write config classes and validate required DATABASE_URL and SECRET_KEY at startup.' },
    { title: 'Request And App Context', focus: 'request, current_app, g, context locals, teardown behavior, and why these are not simple globals', task: 'store a request-scoped correlation id on g and use it in a response header.' },
    { title: 'Routing And Converters', focus: 'route rules, variable converters, endpoint names, url_for, methods, and trailing slash behavior', task: 'define CRUD routes for projects and generate URLs with url_for.' },
    { title: 'Request Parsing', focus: 'request.args, request.get_json, form data, files, headers, and defensive parsing', task: 'parse query params and JSON body without trusting missing or malformed input.' },
    { title: 'Response Construction', focus: 'jsonify, response tuples, status codes, headers, cookies, streaming responses, and content types', task: 'return stable JSON responses for success, validation failure, and not found.' },
    { title: 'Error Handlers', focus: 'register_error_handler, HTTPException, domain errors, 404/405, and consistent error envelopes', task: 'map custom domain exceptions into a stable JSON error response.' },
    { title: 'Before And After Request Hooks', focus: 'before_request, after_request, teardown_request, timing, auth, request ids, and cleanup', task: 'add request timing and request id hooks around every request.' },
    { title: 'Teardown And Resource Cleanup', focus: 'teardown_appcontext, closing DB sessions, releasing resources, and avoiding connection leaks', task: 'fix a leaked SQLAlchemy session with request teardown cleanup.' },
    { title: 'Flask-SQLAlchemy Setup', focus: 'extension initialization, db.init_app, model imports, session scope, and metadata creation pitfalls', task: 'configure Flask-SQLAlchemy inside an app factory without circular imports.' },
    { title: 'SQLAlchemy Models', focus: 'models, columns, relationships, constraints, indexes, and source-of-truth invariants', task: 'model User, Project, Membership, and Invitation with constraints.' },
    { title: 'Migrations With Flask-Migrate', focus: 'Alembic integration, migration review, autogenerate limits, expand/contract, and production rollout', task: 'plan a safe migration adding a required slug field to projects.' },
    { title: 'Transactions In Flask', focus: 'session lifecycle, commit/rollback, short transactions, service boundaries, and after-commit side effects', task: 'write an invite flow that rolls back on failure and sends no email before commit.' },
    { title: 'Repository And Service Layer', focus: 'keeping routes thin, moving business logic into services, and isolating persistence behind repositories', task: 'refactor a route into request parser, service, repository, and response mapper.' },
    { title: 'Validation Strategies', focus: 'manual validation, Marshmallow/Pydantic-style schemas, field errors, normalization, and rejecting untrusted JSON', task: 'validate a create-project body and return field-level errors.' },
    { title: 'Serialization Boundaries', focus: 'response DTOs, datetime, Decimal, UUID, hidden fields, and stable public contracts', task: 'serialize a project with members without leaking internal columns.' },
    { title: 'Authentication With Sessions', focus: 'Flask sessions, cookies, SECRET_KEY, server-side sessions, secure flags, and login/logout flows', task: 'design session login/logout with HttpOnly, Secure, and SameSite cookie settings.' },
    { title: 'JWT Auth In Flask', focus: 'Bearer parsing, signature validation, issuer/audience/expiry checks, and request principal mapping', task: 'write auth middleware that verifies a JWT before trusting user id claims.' },
    { title: 'Authorization Decorators', focus: 'login_required, custom decorators, resource-level checks, and failing closed', task: 'protect project routes so only members can read or update.' },
    { title: 'CSRF And Browser Security', focus: 'CSRF tokens, SameSite cookies, Flask-WTF concepts, API/browser distinctions, and trusted origins', task: 'explain when Flask APIs need CSRF protection and when token auth changes the risk.' },
    { title: 'CORS In Flask', focus: 'origin allowlists, credentials, preflight, flask-cors configuration, and browser enforcement limits', task: 'configure credentialed CORS for one frontend origin.' },
    { title: 'File Uploads', focus: 'multipart parsing, secure_filename limits, size caps, MIME distrust, temp files, and object storage handoff', task: 'design a safe document upload endpoint.' },
    { title: 'Streaming And Generators', focus: 'stream_with_context, generator responses, memory pressure, and worker occupancy', task: 'stream a large CSV response and explain the worker tradeoff.' },
    { title: 'Background Jobs With Celery/RQ', focus: 'producer/worker split, broker config, retries, idempotency, and job payload design', task: 'enqueue an invite email job safely after committing the invitation.' },
    { title: 'Caching In Flask', focus: 'per-request cache, Flask-Caching, Redis, cache keys, TTLs, invalidation, and stale data risks', task: 'cache a project summary and invalidate it on update.' },
    { title: 'Rate Limiting', focus: 'Flask-Limiter concepts, per-IP/per-user keys, Redis storage, login protection, and fail-open/fail-closed tradeoffs', task: 'design login and API-key rate limits.' },
    { title: 'Testing With pytest', focus: 'app fixtures, test_client, app context, database fixtures, and deterministic config', task: 'write pytest fixtures for app, client, and isolated test database.' },
    { title: 'Testing Routes', focus: 'Flask test_client, JSON assertions, auth fixtures, status codes, and error branches', task: 'test create project success, validation failure, auth failure, and permission failure.' },
    { title: 'Testing Services', focus: 'pure service tests, fake repositories, fake email clients, and avoiding framework-heavy tests for business logic', task: 'unit test invitation service with fake repository and fake mailer.' },
    { title: 'Integration Tests With DB', focus: 'real database tests, transactions, cleanup, migrations, factories, and avoiding test order coupling', task: 'prove membership uniqueness against the database.' },
    { title: 'CLI Commands', focus: 'Flask CLI, app context in commands, backfills, dry-run flags, and idempotent maintenance scripts', task: 'write a dry-run backfill command for missing project slugs.' },
    { title: 'Logging With Context', focus: 'structured logs, request ids, user ids, endpoint names, duration, and exception context', task: 'add JSON request logs with request id and status code.' },
    { title: 'Metrics And Instrumentation', focus: 'request counters, duration histograms, DB timing, worker metrics, labels, and low cardinality', task: 'define metrics for Flask endpoints and background jobs.' },
    { title: 'Tracing Flask Requests', focus: 'spans, request id propagation, downstream calls, DB spans, and error attributes', task: 'trace a request that calls the database and an email provider.' },
    { title: 'Health And Readiness', focus: 'liveness vs readiness, dependency checks, startup readiness, and load balancer behavior', task: 'write /livez and /readyz semantics for a Flask service.' },
    { title: 'Gunicorn Worker Tuning', focus: 'sync workers, threads, timeouts, preload, max requests, memory leaks, and CPU/IO workload fit', task: 'choose Gunicorn settings for an IO-heavy Flask API.' },
    { title: 'Deployment Config', focus: 'environment config, reverse proxy headers, ProxyFix, HTTPS, static assets, and platform differences', task: 'configure Flask safely behind a reverse proxy.' },
    { title: 'Dockerizing Flask', focus: 'multi-stage-ish Python images, pinned dependencies, non-root users, Gunicorn command, and runtime env', task: 'write a production Dockerfile and start command for Flask.' },
    { title: 'Security Headers', focus: 'HSTS, CSP basics, frame options, content sniffing, cookies, and Flask-Talisman-style defaults', task: 'add safe security headers and explain each one.' },
    { title: 'Secrets Management', focus: 'SECRET_KEY, database URLs, provider tokens, .env boundaries, secret managers, and rotation', task: 'write a config policy for local/staging/prod secrets.' },
    { title: 'ORM Performance', focus: 'N+1 queries, eager loading, indexes, pagination, query timing, and response DTO design', task: 'diagnose a slow project list endpoint with N+1 queries.' },
    { title: 'Pool Exhaustion Debugging', focus: 'SQLAlchemy pool sizing, leaked sessions, long transactions, worker count, and DB saturation', task: 'triage QueuePool timeout errors under load.' },
    { title: 'Memory Leak Debugging', focus: 'global caches, request data retention, extension state, worker recycling, and profiling', task: 'diagnose a Flask app whose memory grows after every request.' },
    { title: 'Incident Response', focus: 'logs, metrics, traces, deploy correlation, dependency failures, rollback, and mitigation', task: 'write a triage plan for rising 5xx and p99 latency.' },
    { title: 'Flask Launch Review', focus: 'config, app factory, sessions, auth, validation, DB sessions, migrations, jobs, tests, observability, and deployment', task: 'perform a production launch review for a Flask API.' },
    { title: 'Flask Capstone API', focus: 'building a complete Flask/Postgres/Redis/Celery backend with auth, projects, invitations, tests, and production docs', task: 'build and defend a Flask project-management API end to end.' },
    { title: 'Flask Mastery Oral Exam', focus: 'explaining Flask backend architecture from WSGI request to database transaction to worker side effect', task: 'defend every major Flask backend choice: app factory, DB sessions, auth, jobs, tests, deploy, and observability.' },
  ]),
